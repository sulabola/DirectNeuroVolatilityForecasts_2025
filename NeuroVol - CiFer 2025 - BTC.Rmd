...
BTC <- getSymbols("BTC-USD", src = "yahoo", from = start.date, to = end.date, source="yahoo", auto.assign=FALSE, return.class="xts")
Asset <- na.omit(BTC)
...
rho <- rho.cal(r)
vol <- sqrt(365)*observed.vol(as.numeric(r))
...
```{r}
Train <- Asset.Vol[(1:864),]
Test <- Asset.Vol[(865:length(vol)),]
...
Train.Cond.Var <- Cond.Variance[(1:864),]
head(Train.Cond.Var)
tail(Train.Cond.Var)
...
ggplot(data = VolForecasts, aes(x = as.Date(Date))) +
  ...
  theme_minimal() +
  theme(legend.position = c(0.7, 0.80))  # Place the legend at coordinates (0.9, 0.9) within the plotting area
```
...
ggplot(data = VolForecasts.Bidirectional, aes(x = as.Date(Date))) +
  ...
  theme_minimal() +
  theme(legend.position = c(0.7, 0.80))  # Keep the legend at the same position
